...
class StoreRequest(models.Model):
    ...
    def _is_allowed_check(self):
        for rec in self:
            rec.is_allowed = rec.env.user.has_group('pre_mrp.group_shift_leader')
    ...
    def create_purchase_request(self):
        ...
        wizard = self.env['store.request.purchase.wizard'].create({
            ...
            }) for line in self.request_lines if line.quantity > line.qty_available]
        })
    ...
    def create_issue_voucher(self):
        ...
        wizard = self.env['store.request.siv.wizard'].create({
            ...
            }) for line in self.request_lines if line.qty_available > 0]
        })
...
class StoreRequestLine(models.Model):
    ...
    uom_id = fields.Many2one('uom.uom', string="Unit of Measure", default= lambda self: self.product_id.uom_po_id)
