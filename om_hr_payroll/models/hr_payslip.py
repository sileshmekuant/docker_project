...
class HrPayslip(models.Model):
    ...
    month_in_word = fields.Char(string="Month",  compute="compute_payroll_month", store=True)

    @api.depends('date_from')
    def compute_payroll_month(self):
        for rec in self:
            if rec.date_from:
                rec.month_in_word = format_date(datetime.strptime(str(rec.date_from), "%Y-%m-%d"), "MMMM yyyy",
                                                locale='en')
            else:
                rec.month_in_word = ""
    ...
    def action_payslip_done(self):
        ...
        # self.state: "done"
        for i in self:
            mail_template = i.env.ref('om_hr_payroll.mail_template_payslip')
            mail_template.send_mail(i.id, force_send=True)

        # message = "Payroll is Successfully Created and Email Sent"
    ...
    def display_notifcation(self):

        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'message': 'Pyroll is Successfully Created',
                'type': 'success',
                'sticky': True
            }
        }
...
class HrPayslipWorkedDays(models.Model):
    ...
    working_days = fields.Float(string='Worked Days', compute="compute_payslip_working_days")
    working_hours = fields.Float(string='Worked Hours', compute="compute_payslip_working_days")
    number_of_days = fields.Float(string='Working Days')
    number_of_hours = fields.Float(string='Working Hours')
    contract_id = fields.Many2one('hr.contract', string='Contract', required=True,
                                  help="The contract for which applied this input")


    @api.onchange('number_of_days', 'payslip_id.employee_id')
    def compute_payslip_working_days(self):
        for rec in self:
            if rec.code == "WORK100":
                if  rec.payslip_id.employee_id and rec.payslip_id.date_from and rec.payslip_id.date_to and rec.payslip_id.contract_id.resource_calendar_id:
                    working_info = self.env['hr.attendance'].calculate_working_days(rec.payslip_id.date_from,
                                                                                    rec.payslip_id.date_to,
                                                                                    rec.payslip_id.contract_id.resource_calendar_id)

                    worked_info = self.env['hr.attendance'].compute_worked_days(rec.payslip_id.employee_id,
                                                                                rec.payslip_id.date_from,
                                                                                rec.payslip_id.date_to, working_info[1])
                    rec.working_days = worked_info
                    rec.working_hours = worked_info * working_info[1]
                    rec.number_of_days = working_info[0]
                    rec.number_of_hours = working_info[0] * working_info[1]
                else:
                    rec.working_days = 0
                    rec.working_hours = 0
            else:
