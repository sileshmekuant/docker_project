...
class BiometricDeviceDetails(models.Model):
                    ...
                    for each in attendance:
                        ...
                        for uid in user:
                            if uid.user_id == each.user_id:
                                ...
                                if get_user_id:
                                    print("============================== users =====================0",get_user_id,get_user_id.name)
    ...
    def action_resync_hr_attendance(self):
        """Synchronize HR Attendance from zk.machine.attendance based on punch type.
        Ensures one attendance record per employee per day and fills missing check-ins/check-outs."""
        
        zk_attendance_records = self.env['zk.machine.attendance'].search([], order='punching_time asc')
        ...
        for record in zk_attendance_records:
            ...
            # Convert to date for grouping by day
            punch_dt = fields.Datetime.from_string(record.punching_time)
            punch_date = punch_dt.date()

            # Search for existing attendance record for that employee on the same day
            ...
            if record.punch_type == '0':  # Check-In
                if attendance:
                    # If already has a check-in, skip creating a new one
                    continue
                else:
                ...
                if attendance:
                    # Update check-out if not filled yet or if this time is later
                    if not attendance.check_out or fields.Datetime.from_string(record.punching_time) > fields.Datetime.from_string(attendance.check_out):
                        attendance.write({'check_out': record.punching_time})
                else:
                    # No check-in exists → create a record with both check-in/out as same
                    hr_attendance_model.create({
                        'employee_id': emp.id,
                        'check_in': record.punching_time,
        ...
        return {
            ...
            'tag': 'display_notification',
            'params': {
                'message': 'HR attendance successfully re-synced and updated based on punch type',
                'type': 'success',
                'sticky': False
            }
        }
